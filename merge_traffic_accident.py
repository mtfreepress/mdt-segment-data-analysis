...
# 365.20 instead of .25 because of leap years in 5-year span. 
    ...
    for _, row in filtered.iterrows():
        ...
        props = {
            ...
            'CORR_MP_FLOAT': float(row.get('CORR_MP_FLOAT')) if pd.notna(row.get('CORR_MP_FLOAT')) else '',
            'CORR_ENDMP_FLOAT': float(row.get('CORR_ENDMP_FLOAT')) if pd.notna(row.get('CORR_ENDMP_FLOAT')) else '',
